 ...
 <body>
     ...
     <script type="module">
         ...
         let puzzleImage = null; // The image loaded for the puzzle
         let puzzlePieces = []; // Array to store puzzle pieces
         let rows = 3; // Default rows
         let cols = 3; // Default columns
         let pieceWidth;
         let pieceHeight;
         ...
         // Variables to store the dimensions and offsets of the drawn image on the main canvas
         let drawnImage = {
             width: 0,
             height: 0,
             offsetX: 0,
         ...
         let gameActive = false; // Game active state (true for interaction, false to disable)
         ...
         /**
          * Starts the countdown timer for the puzzle.
          * @param {number} durationInSeconds - The duration of the timer in seconds.
          */
         function startTimer(durationInSeconds) {
             ...
             timerInterval = setInterval(() => {
                 ...
                 if (timeRemaining <= 0) {
                     ...
                     gameActive = false; // Disable game interaction
                     showMessage('Tempo Esgotado! O quebra-cabeça foi bloqueado.', 'error');
                     drawPuzzle(); // Redraw to ensure no active piece border if dragging
                 }
             }, 1000);
         }

         /**
          * Adjusts the size of the main canvas and preview canvas to be responsive.
          * Redraws the puzzle and preview after resizing.
          */
         function resizeCanvas() {
             const rect = canvas.getBoundingClientRect();
             canvas.width = rect.width;
             canvas.height = rect.height;
             ctx.imageSmoothingEnabled = true; // Ensure image smoothing is enabled for better quality
             drawPuzzle(); // Redraw the puzzle after resizing
             drawPreview(); // Redraw the preview as well
         }

         // Initialize canvas size when the window loads
         ...
         /**
          * Loads an image from a given source (URL or DataURL).
          * @param {string} src - The source URL or DataURL of the image.
         ...
         /**
          * Draws the full image on a target canvas context, scaling it to fit while maintaining aspect ratio.
          * @param {HTMLImageElement} img - The image to draw.
          * @param {CanvasRenderingContext2D} [targetCtx=ctx] - The context to draw on (defaults to main canvas context).
          */
         function drawFullImage(img, targetCtx = ctx) {
             ...
             if (imgAspectRatio > canvasAspectRatio) {
                 // Image is wider in relation to its height than the canvas, scale based on width
                 scale = targetCtx.canvas.width / imgWidth;
             } else {
                 // Image is taller in relation to its width than the canvas, scale based on height
             ...
             // If drawing on the main canvas, store the dimensions and offsets for puzzle logic
             if (targetCtx === ctx) {
         ...
         /**
          * Draws a grid over an image on a specified canvas context.
          * @param {CanvasRenderingContext2D} context - The canvas rendering context.
          * @param {number} currentRows - Number of rows for the grid.
          * @param {number} currentCols - Number of columns for the grid.
          * @param {number} currentPieceWidth - Calculated width of each piece.
          * @param {number} currentPieceHeight - Calculated height of each piece.
          * @param {number} offsetX - X offset for drawing the grid.
          * @param {number} offsetY - Y offset for drawing the grid.
          * @param {number} totalWidth - Total width of the area to draw the grid.
          * @param {number} totalHeight - Total height of the area to draw the grid.
          */
         ...
         /**
          * Draws the divided image with grid in the preview canvas.
          */
         function drawPreview() {
             if (puzzleImage) {
                 const previewRows = parseInt(rowsInput.value) || 3; // Default to 3 if invalid
                 const previewCols = parseInt(colsInput.value) || 3; // Default to 3 if invalid
         ...
         /**
          * Creates and shuffles the puzzle pieces, then starts the game.
          */
         function createPuzzlePieces() {
             ...
             let selectedTimeMinutes = parseInt(timeInput.value);

             if (isNaN(rows) || rows < 2 || rows > 10) {
                 showMessage('Número de linhas deve ser entre 2 e 10.', 'error');
                 return;
             }
             if (isNaN(cols) || cols < 2 || cols > 10) {
                 showMessage('Número de colunas deve ser entre 2 e 10.', 'error');
                 return;
             }
             if (isNaN(selectedTimeMinutes) || selectedTimeMinutes < 1 || selectedTimeMinutes > 10) {
                 showMessage('O tempo deve ser um número válido (mínimo 1 minuto, máximo 10 minutos).', 'error');
                 return;
             }
             ...
             // Draw the full image on the main canvas and then the grid
             ...
             // Small delay for the user to see the image with the grid before shuffling
             setTimeout(() => {
                 puzzlePieces = [];
                 ...
                 for (let r = 0; r < rows; r++) {
                     for (let c = 0; c < cols; c++) {
                         ...
                         puzzlePieces.push({
                             ...
                             originalRow: r,
                             originalCol: c,
                             currentRow: r, // Initial logical position (for shuffling later)
                             currentCol: c,
         ...
         function shufflePieces() {
             ...
             // Update current drawing positions of pieces after shuffling
          ...
          * Handles drawing the full image with grid if the puzzle hasn't started yet.
          */
         function drawPuzzle() {
             ...
             if (draggingPiece && draggingPiece.imageData) {
         ...
         /**
          * Determines which puzzle piece is at the current mouse/touch position.
          * @param {MouseEvent|TouchEvent} e - The event object.
          * @returns {{piece: object, index: number}|null} The piece object and its index, or null if no piece is found.
          */
         function getPieceAtMouse(e) {
             if (!gameActive) return null; // Prevent interaction if game is not active

             const rect = canvas.getBoundingClientRect();
             // Get clientX/Y from mouse event or first touch point
             ...
             for (let i = 0; i < puzzlePieces.length; i++) {
                 ...
                 // Check if mouse/touch is within the current drawing position of the piece
         ...
         canvas.addEventListener('mouseup', (e) => {
             ...
             if (draggingPiece) {
                 ...
                 // Find the piece that is currently in the target cell
                 ...
                 if (targetPiece && targetPiece !== draggingPiece) {
                     ...
                     // If no target piece or it's the same piece, just snap back to its current logical position
                     draggingPiece.x = drawnImage.offsetX + (draggingPiece.currentCol * pieceWidth);
                     draggingPiece.y = drawnImage.offsetY + (draggingPiece.currentRow * pieceHeight);
                 }
         ...
         canvas.addEventListener('mouseout', () => {
             if (!gameActive) return;
             if (draggingPiece) {
                 // If the mouse leaves the canvas while dragging, snap the piece back
                 canvas.style.cursor = 'grab';
                 draggingPiece.x = initialPieceX; // Revert to initial position
                 draggingPiece.y = initialPieceY;
                 draggingPiece = null;
                 draggingPieceIndex = -1;
                 drawPuzzle();
             }
         ...
         /**
          * Checks if the puzzle is complete. If so, displays a success message and stops the game.
          */
